...
function debug_utils.print_ts_delta(start_time, ...)
    ...
    local arg = { ... }
    table.insert(arg, 1, string.format('[ t = %.3f, dt = %.3f ]', ts, delta))
...
function debug_utils.print_debug_eval(ca_name, ca_score, start_time, ...)
    if debug_utils.show_debug('eval') then
        local prefix = string.format('T%d S%d %-20s %7d', wesnoth.current.turn, wesnoth.current.side, ca_name .. ' eval', ca_score)
        debug_utils.print_ts_delta(start_time, prefix, ...)
    end
end

function debug_utils.print_debug_exec(ca_name, ...)
    if debug_utils.show_debug('exec') then
        local prefix = string.format('T%d S%d %-27s', wesnoth.current.turn, wesnoth.current.side, ca_name .. ' exec')
        std_print(prefix, ...)
    end
end
...
function debug_utils.put_gm_labels(map, key, cfg)
    -- Take a gamestate map (in the format defined in gamestate_map.lua) and put
    ...
    for x,y,data in GM.iter(map) do
        local out = data[key]
...
function debug_utils.show_gm_with_message(map, key, text, cfg)
    -- @cfg: optional table with display configuration parameters:
    --   @x,@y: coordinates to scroll to; if omitted, no scrolling is done
    --     Scrolling is roughly to middle between the center of gravity of the map and the specified coordinate
    ...
    debug_utils.put_gm_labels(map, key, cfg)
...
function debug_utils.dbms(lua_var, clear, name, onscreen, wrap, only_return)
        ...
        if not only_return then
                std_print(result)
                if wesnoth and ((not clear) or (clear and (clear ~= -1))) then wesnoth.interface.add_chat_message("dbms", result) end;
        end
        local continue = true
